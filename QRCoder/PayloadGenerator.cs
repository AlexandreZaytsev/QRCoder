...
#pragma warning restore CS0612
        ...
        #region old version RussiaPaymentOrder
        /*
        public class RussiaPaymentOrder1 : Payload
        {
            ...
            public RussiaPaymentOrder1(string name, string personalAcc, string bankName, string BIC, string correspAcc, OptionalFields optionalFields = null, CharacterSets characterSet = CharacterSets.utf_8) : this()
            {
        ...
        */
        ...
        public class RussiaPaymentOrder : Payload //PayloadGenerator.Payload
        {
            ...
            public byte[] ToBytes()
            {
                ...
                //Create the payload string
                string ret = $"ST0001" + ((int)characterSet).ToString();

                //Add mandatory fields, if filled
                var MandatoryFields = GetMandatoryFieldsAsList();
                if (MandatoryFields.Count > 0)
                    ret += separator + $"{string.Join(" " + separator, MandatoryFields.ToArray())}";
                ret += separator;
                ...
                //Add optional fields, if filled
                var optionalExtFieldsList = GetOptionalExtFieldsAsList();
                if (optionalExtFieldsList.Count > 0)
                    ret += $"|{string.Join("|", optionalExtFieldsList.ToArray())}";
                ret += separator;
                ...
                var cp = this.characterSet.ToString().Replace("_", "-");
                byte[] bytesOut = Encoding.Convert(Encoding.UTF8, Encoding.GetEncoding(cp), Encoding.UTF8.GetBytes(ret));
                if (bytesOut.Length > 600)//300)
                    throw new RussiaPaymentOrderException($"Data too long. Payload must not exceed 600 bytes, but actually is {bytesOut.Length} bytes long. Remove additional data fields or shorten strings/values.");
                return bytesOut;
            }
            ...
            private string DetermineSeparator()
            {
                ...
                var mandatoryValues = GetMandatoryFieldsAsList();
                var optionalValues = GetOptionalFieldsAsList();
                var optionalExtValues = GetOptionalExtFieldsAsList();
                ...
                foreach (var sepCandidate in separatorCandidates)
                {
                    if (!mandatoryValues.Any(x => x.Contains(sepCandidate)) &&
                        ...
                        !optionalExtValues.Any(x => x.Contains(sepCandidate))
                        )
                        return sepCandidate;
                }
                throw new RussiaPaymentOrderException("No valid separator found.");
            ...
            {
#if NETSTANDARD1_3
                return mFields.GetType().GetRuntimeProperties()//.GetRuntimeFields()
                        .Where(field => field.GetValue(mFields) != null)
                        .Select(field => {
                            var objValue = field.GetValue(mFields);
                            var value = field.PropertyType.Equals(typeof(DateTime?)) ? ((DateTime)objValue).ToString("dd.MM.yyyy") : objValue.ToString();
                        ...
                        .ToList();
#else
                return mFields.GetType().GetProperties()//.GetFields()
                        .Where(field => field.GetValue(mFields, null) != null)
                        .Select(field => {
                            var objValue = field.GetValue(mFields, null);
                            var value = field.PropertyType.Equals(typeof(DateTime?)) ? ((DateTime)objValue).ToString("dd.MM.yyyy") : objValue.ToString();
                            return $"{field.Name}={value}";
                        })
                        .ToList();
            ...
            /// <summary>
            /// Takes all optional Ext fields that are not null and returns their string represantion
            /// </summary>
            /// <returns>A List of strings</returns>
            private List<string> GetOptionalExtFieldsAsList()
            {
#if NETSTANDARD1_3
                return oExtFields.GetType().GetRuntimeProperties()
                        .Where(field => field.GetValue(oExtFields) != null)
                        .Select(field => {
                            var objValue = field.GetValue(oExtFields, null);
                            var value = field.PropertyType.Equals(typeof(DateTime?)) ? ((DateTime)objValue).ToString("dd.MM.yyyy") : objValue.ToString();
                            return $"{field.Name}={value}";
                        })
                        .ToList();
#else
                return oExtFields.GetType().GetProperties()
                        .Where(field => field.GetValue(oExtFields, null) != null)
                        .Select(field => {
                            var objValue = field.GetValue(oExtFields, null);
                            var value = field.PropertyType.Equals(typeof(DateTime?)) ? ((DateTime)objValue).ToString("dd.MM.yyyy") : objValue.ToString();
                            return $"{field.Name}={value}";
            ...
            public class MandatoryFields
            {
                ...
                /// <summary>
                /// Name of the payee (Наименование получателя платежа)
                /// <para>Сумма платежа, в копейках</para>
                /// </summary>
                public string Name
                {
                    get { return _name; }
                    set { _name = ValidateInput(value, "Name", @"^.{1,160}$"); }
                }

                private string _personalAcc;
                /// <summary>
                /// Beneficiary account number
                /// <para>Номер счета получателя платежа</para>
                /// </summary>
                public string PersonalAcc
                {
                    get { return _personalAcc; }
                    set { _personalAcc = ValidateInput(value, "PersonalAcc", @"^[1-9]\d{4}[0-9ABCEHKMPTX]\d{14}$"); }
                }

                private string _bankName;
                /// <summary>
                /// Name of the beneficiary's bank
                /// <para>Наименование банка получателя платежа</para>
                /// </summary>    
                public string BankName
                {
                    get { return _bankName; }
                    set { _bankName = ValidateInput(value, "BankName", @"^.{1,45}$"); }
                }
                ...
                {
                    get { return _bic; }
                    set { _bic = ValidateInput(value, "BIC", @"^\d{9}$"); }
                }

                private string _correspAcc;
                /// <summary>
                /// Box number / account payee's bank
                /// <para>Номер кор./сч. банка получателя платежа</para>
                /// </summary>    
                public string CorrespAcc
                {
                    get { return _correspAcc; }
                    set { _correspAcc = ValidateInput(value, "CorrespAcc", @"^[1-9]\d{4}[0-9ABCEHKMPTX]\d{14}$"); }
                }
            ...
            public class OptionalFields
            {
                ...
                //https://rosswift.ru/doc/gate_way_swift0.2.pdf  (3.2)
                //https://www.garant.ru/products/ipo/prime/doc/71023286/
                //Унифицированные форматы электронных банковских сообщений Банка России. Обмен с клиентами Банка России
                //http://cbr.ru/development/Formats/
